...
#' @param new_colname A string that is the new name of the column 
...
#TODO: Throw warning if col_toreplace == new_colname
replace_idcol <- function(dataset, col_toreplace, new_colname = "replacement.ID", lookup){
  # save the names of the original dataset
  df_names <- names(dataset)
  
  # Set name for the replacement column
  names(lookup)[2] <- new_colname
  ...
  df_names[which(df_names == col_toreplace)] <- new_colname
